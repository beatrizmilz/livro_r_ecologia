...
Para realizar as **manipulações nos dados** existe uma série de funções, para diversas finalidades: manipulação de uma tabela, manipulação de duas tabela, agrupamento, funções de vetores, além de outras [funções específicas](https://dplyr.tidyverse.org/reference/index.html).
...
- `group_by()`: agrupamento das linhas pelos valores das colunas
- `summarise()`: resume os dados através de funções, podendo considerar valores das colunas
- `*_join()`: séria de funções junta dados de duas tabelas através de uma coluna chave

Todas as funções seguem a mesma **sintaxe**, de modo que o `tibble` é sempre o **primeiro argumento** das funções verbais, seguido de um **pipe** e pelo o **nome da função**. Isso permite o **encademanto** de várias operações consecutivas.
...
Além de `data.frames` e `tibbles`, a manipulação pelo formato *dplyr* torna o trabalho com outros formatos de classes e dados acessível e eficiente.
...
Podemos ainda **renomear colunas** facilmente com a função `dplyr::rename()`, onde primeiramente informamos o nome que queremos que a coluna tenha, seguido do operador `=` e a coluna do nosso dado. É fundamental mencionar as variações da função `dplyr::rename()` pelo sufixos: `rename_with()`, `dplyr::rename_all()`, `dplyr::rename_at()`, `dplyr::rename_if()` e `dplyr::rename_vars()`, para as quais as funções irão atuar dependendo de respostas booleanas para **mudanças no nome de várias colunas de uma vez**.
...
# mudar o nome das colunas todas para miusculas
penguins_rename_with <- penguins %>% 
  dplyr::rename_with(toupper)
```

Outra operação bastante usual dentro da manipulação de dados tabulares é a **seleção de colunas**. Podemos fazer essa operação com a função `dplyr::select()`, que seleciona colunas pelo nome ou pela posição. Aqui há uma séria de possibilidades de seleção de colunas, que listamos na Tabela (\@ref(tab:tab-dplyr-select)). Além disso, é fundamental destacar as variações da função `dplyr::select()`: `dplyr::select_all()`, `dplyr::select_at()`, `dplyr::select_if()` e `dplyr::select_vars()`, para as quais as funções irão atuar dependendo de respostas booleanas para **seleção de várias colunas de uma vez**.

```{r tab-dplyr-select, echo=FALSE}
kable(data.frame(
 Função = c(
  "`:`", 
  "`!`", 
  ...
  "`where()`"),
 Descrição = c(
   "Para selecionar um intervalo de colunas consecutivas", 
   "Para tomar o complemento de um conjunto de colunas", 
   "Para selecionar a intersecção ou a união de dois conjuntos de colunas", 
   "Para combinar seleções", 
   "Corresponde a todas as colunas", 
   "Seleciona a última coluna",
   "Começa com um prefixo", 
   "Termina com um sufixo", 
   "Contém um caracter literal", 
   "Corresponde a uma expressão regular",
   "Corresponde a um intervalo numérico", 
   "Corresponde aos nomes das variáveis em um vetor de caracteres. Todos os nomes devem estar presentes, senão um erro fora dos limites será gerado",
   "O mesmo que `all_of()`, exceto que nenhum erro é lançado para nomes que não existem.",
   "Aplica uma função a todas as variáveis e seleciona aquelas para as quais a função retorna TRUE")),
 align = "c",
 caption = "Visão geral dos recursos da função `dplyr::select()`.")
```

```{r}
# selecionar as colunas por nomes
penguins_select <- penguins %>% 
  dplyr::select(bill_length_mm:body_mass_g)

# selecionar as colunas por padrap
...
Uma das operações mais úteis dentre as operações para colunas é **adicionar ou atualizar os valores** de colunas. Para essa operação, usaremos a função `dplyr::mutate()`. Essa função irá adiconar colunas ou atualizar uma coluna fazendo operações em outras colunas. Existe uma série de funções que podemos utilizar para fazer essas operações. Destacamos aqui a função `dplyr::across()`, que aplica uma função em múltiplas colunas. Além dessa função, podemos ainda usar os argumentos **.before** e **.after** para indicar onde a nova coluna deve ficar. Temos ainda o parâmetro **.keep** com diversas possibilidades de manter colunas depois de usar a função `dplyr::mutate()`. Por fim, é fundamental mencionar as variações dos sufixos da função `dplyr::mutate()`: `dplyr::mutate_all()`, `dplyr::mutate_at()`, `dplyr::mutate_if()` e `dplyr::mutate_each()`, para as quais as funções irão atuar dependendo de respostas booleanas, **adicionando ou atualizando valores para várias colunas de uma vez**.
...
# modificar colunas
penguins_mutate_modificar <- penguins %>% 
  dplyr::mutate(across(!c(bill_length_mm, bill_depth_mm, flipper_length_mm,
                          body_mass_g), as.character))
```

Além de operações em colunas, podemos fazer **operações em linhas**. Vamos começar com a **reordenação das linhas** com base nos valores das colunas. Para essa operação, usamos a função `dplyr::arrange()`. Podemos reordenadar por uma ou mais colunas de forma crescente ou de forma decrescente usando a função `desc()` ou o operador `-`. Da mesma forma que as funções anteriores, há variações dos sufixos da função `dplyr::arrange()`: `dplyr::arrange_all()`, `dplyr::arrange_at()` e `dplyr::arrange_if()`, para as quais as funções irão atuar dependendo de respostas booleanas, **ordenando linhas para várias colunas de uma vez**.
...
Uma das principais e mais usuais operações que podemos realizar em linhas é a seleção de linhas através do **filtro por valores de uma ou mais colunas**. Iremos utilizar a função `dplyr::filter()`. Para realizar os filtros iremos utilizar grande parte dos operadores relacionais e lógicos que listamos na Tabela \@ref(tab:tab-operadores), especialmente os lógicos para combinações de filtros em mais de uma coluna. Além desses operadores, podemos utilizar a função `is.na()` para filtros em elementos faltantes, e as funções `dplyr::between()` e `dplyr::near()`, para valores filtros entre valores, e para valores próximos com certa tolerência, respectivamente. Por fim, existem variações dos sufixos da função `dplyr::filter()`: `dplyr::filter_all()`, `dplyr::filter_at()` e `dplyr::filter_if()`, para as quais as funções irão atuar dependendo de respostas booleanas para **filtro das linhas para mais de uma coluna**.
...
Além da selecção de linhas por filtros, podemos fazer a **seleção das linhas por intervalos**, indicando quais **linhas desejamos**, usando a função `dplyr::slice()`, e informando o argumento **n** para o número da linha ou intervalo. Essa função possui
variações no sufixo muito interessantes: `dplyr::slice_head()` e `dplyr::slice_tail()` seleciona as primeiras e últimas linhas, `dplyr::slice_min()` e `dplyr::slice_max()` seleciona linhas com os maiores e menores valores de uma coluna, e `dplyr::slice_sample()` seleciona linhas aleatoriamente.
...
A última operação que apresentaremos aqui será a **retirada de linhas com valores repetidos com base nos valores de colunas** utilizandos a função `dplyr::distinct()`. Essa função por padrão retorna apenas a coluna utilizada para retirar as linhas com valores repetidos, sendo necessário acrescentar o argumento **.keep_all** para retornar todas as colunas. Por fim, existem variações dos sufixos da função `dplyr::distinct()`: `dplyr::distinct_all()`, `dplyr::distinct_at()`, `dplyr::distinct_if()` e `dplyr::distinct_prepare()`, para as quais as funções irão atuar dependendo de respostas booleanas para **retirar linhas com valores repetidos para mais de uma coluna**.
...
Também podemos fazer **contagens resumos dos nossos dados**, utilizando para isso a função `dplyr::count()`. Essa função irá contar valores de uma ou mais colunas, semelhante à função Base R `table()`, mas num contexto `tidyverse`.
...
Uma grande parte das operações feita nos dados são realizadas em **grupos definidos por valores de colunas ou variáveis categóricas**. A função `dplyr::group_by()` transforma um `tibble` e converte em um `tibble` agrupado, onde as operações são realizadas "por grupo". Essa função é utilizada geralmente com a função `dplyr::summarise()`, que veremos logo em seguida. O agrupamento não altera a aparência dos dados (além de informar como estão agrupados). A função `dplyr::ungroup()` remove o agrupamento. Há ainda variações dos sufixos da função `dplyr::group_by()`: `dplyr::group_by_all()`, `dplyr::group_by_at()`, `dplyr::group_by_if()`, `dplyr::group_by_prepare()`e `dplyr::group_by_drop_default()`, para as quais as funções irão atuar dependendo de respostas booleanas para **fazer os agrupamentos**.
...
# resumo - if
penguins_summarise_if <- penguins %>% 
  dplyr::group_by(species) %>% 
  summarize_if(is.numeric, mean, na.rm = TRUE)
```

Finalmente, veremos o último conjunto de funções do pacote *dplyr*, a junção. Essas funções fazem a **combinação de pares de conjunto de dados tabulares**. Há dois tipos de junções: junção de mutação e junção de filtragem. A **junção de mutação** combina variáveis (colunas) de duas tabelas. Ele primeiro combina as observações por suas chaves e, em seguida, copia as variáveis de uma tabela para a outra. Existem quatro tipos de junções, que são realizadas pelas funções `dplyr::inner_join()`, `dplyr::left_join()`, `dplyr::full_join()` e `dplyr::right_join()`, e que podem ser representadas na Figura \@ref(fig:fig-r-join).
...
Considerando essa nomenclatura de uma tabela de dados ser `x` e outra ser `y`, temos:

- **inner join**: mantém apenas as observações em `x` em `y`.
- **left join**: mantém todas as observações em `x`.
- **right join**: mantém todas as observações em `y`.
- **full join**: mantém todas as observações em `x` em `y`.

Já a **junção de filtragem**.


### stringr

Pacote para a **manipulação de caracteres ou strings**.

...
- `união(x, y)`: retorna todas as linhas que aparecem em `x`, `y` ou mais dos conjuntos de dados
...
O pacote *stringr* fornece um conjunto de funções para a **manipulação de caracteres ou strings**. O pacote concentra-se nas funções de manipulação mais importantes e comumente usadas. Para função para específicas, recomenda-se usar para o pacote *stringi*, que fornece um conjunto mais abrangente de funções.

As funções do *stringr* podem ser agrupadas em algumas operações para tarefas específicas como correspondência de padrões, espaço em branco, maiúsculas e minúsculas, além de outras funções. 
...
O pacote *forcats* fornece um **conjunto de ferramentas úteis** para **manipular fatores**. Como dito anterioemente, usamos fatores geralmente quando temos **dados categóricos**: variáveis que têm um conjunto de valores fixos e conhecidos. As funções são principalmente para mudar a ordem dos níveis, mudar os valores dos níveis, adicionar e remover níveis, combinar múltiplos níveis, além de outras funções.
...
forcats::fct_recode(penguins$sex, f = "female", m = "male") %>% head()
```
...
```{r}
# inverte os niveis
forcats::fct_rev(penguins$sex) %>% head()
```

forcats::fct_relevel(): especifica a classificação de um ou mais níveis

```{r}
# especifica a classificacao de um nivel
forcats::fct_relevel(penguins$sex) %>% head()
```

forcats::fct_inorder(): ordem em que aparece
```{r}
# ordem em que aparece
penguins_inorder <- penguins %>% 
 dplyr::mutate(forcats::fct_inorder())
```

forcats::fct_infreq(): ordem (decrescente) de frequência
```{r}
# ordem (decrescente) de frequencia
penguins_infreq <- penguins %>% 
 dplyr::mutate(forcats::fct_infreq())
```

forcats::fct_lump(): agregação de níveis raros em um nível
```{r}
# agregacao de niveis raros em um nivel
penguins_lump <- penguins %>% 
 dplyr::mutate(forcats::fct_lump())
```

```{r}
# modificar colunas para caracter
penguins %>% 
  dplyr::mutate(across(where(is.factor), as.character))

# transformar um coluna em fator
penguins_multi_factor <- penguins_caracter %>% 
  dplyr::mutate(species = forcats::as_factor(species))

# transformar varias colunas em fator
penguins_multi_factor <- penguins %>% 

...
A grande necessidade de computação para desenvolver esse novo paradigma colocaram o R e o python como as principais linguagens de programação frente a esses novos desafios. Apesar de não serem as únicas ferramentas utilizadas para esse propósito, elas rapidamente se tornaram uma das melhores escolhas, dado vários fatores como: serem livres, possuírem grandes comunidades contribuidoras, serem linguagens de interpretação (orientadas a objeto) e relativamente fáceis de serem aprendidas e aplicadas.

> B: Eu entendi o que você quis dizer com ser livre, mas acho que seria melhor deixar mais claro.. algo tipo: são de código-aberto e gratuitas,..

Essas mudanças e adaptações da linguagem R para a Ciência de Dados começaram a ser implementadas principalmente devido a um pesquisador: [Hadley Wickham](http://hadley.nz/), que iniciou sua contribuição à comunidade R com o desenvolvimento do já consagrado pacote `ggplot2` [@wickham2016] para a composição de gráficos no R (ver mais no capítulo \@ref(\#graficos)), baseado na gramática de gráficos [@wilkinson2005].

> B: Esse parágrafo dá a ideia de que a linguagem foi adaptada, mas eu não vejo assim. a adaptação da linguagem é o |>, mas o tidyverse não é integrado ao base, então me parece que seria melhor algo como uma "expansão na utilização"...

Depois disso, Wickham dedicou-se ao desenvolvimento do pensamento de uma nova abordagem dentro da manipulação de dados, denominada de **Tidy Data** (Dados organizados) [@wickham2014], na qual focou na limpeza e organização de dados. A ideia, relativamente simples, postula que dados estão `tidy` quando: 1) variáveis estão nas colunas, 2) observações estão nas linhas e 3) valores estão nas células, sendo que para esse último, não deve haver mais de um valor por célula (Figura \@ref(fig:fig-r-dados-tidy)).

> B: aqui tem uma figura mas a figura não aparece aqui...

> B: eu não acho tão legal falar coisas como "relativamente simples,",  acho que fica melhor ir direto.. "a ideia postula que dados sao tidy quando:"


A partir dessas ideias, o [*tidyverse*](https://www.tidyverse.org/) foi operacionalizado no R como uma coleção de pacotes que atuam no workflow comum da ciência de dados: importação, manipulação, exploração, visualização, análise e comunicação de dados e análises [@tidyverse2019] (Figura \@ref(fig:fig-r-tidyverse)). O principal objetivo do *tidyverse* é aproximar a linguagem para melhorar a interação entre ser humano e computador sobre dados, de modo que os pacotes compartilham uma filosofia de design de alto nível e gramática, além da estrutura de dados de baixo nível [@tidyverse2019].

```{r fig-r-tidyverse, fig.align='center', fig.cap="Modelo das ferramentas necessárias em um projeto típico de ciência de dados: importar, organizar, entender (transformar, visualizar, modelar) e comunicar, envolto à essas ferramentas está a programação. Adaptado de: @wickham2017.", out.width="90%", echo=FALSE}
library(knitr)
include_graphics("img/cap04_fig01.png")
```

As principais leituras sobre o tema no R são os artigos [@wickham2014] e [@tidyverse2019], e o livro [@wickham2017], disponível on-line neste [link](https://r4ds.had.co.nz/), além do [site](https://www.tidyverse.org/) que possui muito mais informações.
...
-   [`tibble`](https://tibble.tidyverse.org/): implementou o "tibble", com melhorias na classe `data frame`
...
> B: para ficar padronizado, o tibble seria melhor como "implementa a classe tibble". os ultimos dois tambem poderiam ficar mais padronizados, como "ggplot2: possibilita visualizar dados", "purrr: disponibiliza ferramentas para programação funcional".. são sugestões para deixar a forma parecida.

Além desses principais, fazemos também a menção de outros pacotes que estão dentro dessa abordagem, e que trataremos ainda neste capítulo, em outro momento do livro, ou que você leitor(a) deve se familiarizar.

> B: eu diria que utilizam filosofias semelhantes. alguns dos pacotes abaixo fazem parte do tidyverse e outros não! o readxl, haven, rvest, lubridate, rmarkdown, magrittr são do tidyverse (bom checar todos). talvez essa lista possa ser adicionada em outro lugar? pode assustar quem tá lendo
...
Para instalar os pacotes do *tidyverse*, basta instalar o pacote `tidyverse`.

> b: acho que essa frase poderia ser melhorada. eu nao curto muito usar a palavra "basta".  ex: Para instalar todos os pacotes que fazem parte do tidyverse, instale o pacote: 
...
> B: pessoas desenvolvedoras em vez de "os desenvolvedores""

> B: Pergunta: quem é o público alvo do livro? se é para pessoas iniciantes, nao sei se é de grande ajuda falar do design guide de devs.
...
Dado que possuímos um conjunto de dados e que geralmente esse conjunto de dados estará no formato tabular com umas das extensões: .csv, .txt ou .xlsx, usaremos o pacote `readr` ou `readxl` para importar esses dados para o R.
...
> B: eu acho "classe das colunas" mais claro do que "modos das colunas"
...
Se o arquivo .csv foi criado com separador de decimais sendo `.` e separador de colunas sendo `,`, usamos as funções normalmente. Caso seja criado com separador de decimais sendo `,` e separador de colunas sendo `;`, usaríamos a função `readr::read_csv2()` para importar e `readr::write_csv2()` para exportar nesse formato.

> B: Acho legal comentar que no brasil é mais comum os arquivos terem o ; como separador.
...
> B: Pq vc chama "dados no formato tidyverse"? acho que acaba ficando confuso
...
Para se aprofundar no tema, recomendamos a leitura do capítulo [11 Data import](https://r4ds.had.co.nz/data-import.html) de Wickham & Grolemund (2017).
...
Para se aprofundar no tema, recomendamos a leitura do capítulo [10 Tibbles](https://r4ds.had.co.nz/tibbles.html) de Wickham & Grolemund (2017).
...
Essas operações ainda estão simples, vamos torná-las mais complexas com várias funções compostas. É nesses casos que a propriedade organizacional do uso do pipe emerge: podemos facilmente ver o encadeamento de operações, onde cada função é disposta numa linha.
...
> B: aqui denovo vai o ponto de: para quem vc ta escrevendo o livro? para pessoas aprendizes, acho que não ajuda em nada mostrar os outros pipes. muita coisa para assimilar, talvez seja melhor focar no mais usado.

Para se aprofundar no tema, recomendamos a leitura do capítulo [18 Pipes](https://r4ds.had.co.nz/pipes.html) de Wickham & Grolemund (2017).

A partir da versão do R 4.1 (18/05/2021), o operador pipe se tornou nativo. Entretanto, o operador foi atualizado para `|>`, podendo ser inserido com o mesmo atalho `Ctrl + Shift + M`.

> B: Aqui acho que é legal deixar claro que precisa mudar na IDE qual é o pipe a ser usado - nas configuracoes do RStudio.
...
```{r fig-r-dados-tidy, fig.align='center', fig.cap="As três regras que tornam um conjunto de dados *tidy*. Adaptado de: @wickham2017.", out.width="100%", echo=FALSE}
...
Para realizar diversas transformações nos dados, a fim de ajustá-los ao formato `tidy`, existe uma série de funções, para diversas: unir, separar, valores faltantes (`NA`), pivotar (formato longo para largo), além de outras [funções específicas](https://tidyr.tidyverse.org/reference/index.html).

> B: acho que o "para diversas:" acabou ficando repetido. sugestão:
...
> B: o pivot pode ficar esquisito pois acho que nao foi falado ate então sobre bases largas/longas
...
Primeiramente, vamos exemplificar como juntar e separar colunas. Vamos utilizar a função `tidyr::unite()` para unir as colunas. Há diversos parâmetros para alterar como assa função funciona, entretanto, é importante destacar três deles: `col` nome da coluna que vai receber as colunas unidas, `sep` indicando o caracteree separador das colunas unidas, e `remove` para uma resposta lógica se as colunas unidas são removidas ou não. Vamos unir as colunas "Region" e "Island" na nova coluna "region_island".
...
> B: acho que da pra escrever uns comentarios no codigo para explicar o que ta acontecendo
...
> B: podia mostrar o resultado aqui, para a pessoa ter ideia de um antes e depois

### drop_na() e replace_na()

*Valores faltantes* (`NA`) é um tipo especial de elemento que discutimos na seção YY do Capítulo XX, e são relativamente comuns em conjuntos de dados. Em Base R, vimos algumas formas de lidar com esse tipo de elemento. No formato `tidyverse`, existem várias formas de lidar com eles, mas aqui focaremos nas funções `tidyr::drop_na()` e `tidyr::replace_na()`, para retirar linhas e substitui-los, respectivamente.
...
Para se aprofundar no tema, recomendamos a leitura do capítulo [12 Tidy data](https://r4ds.had.co.nz/tidy-data.html) de Wickham & Grolemund (2017).

## dplyr

O `dplyr` é um pacote que facilita a manipulação de dados, com uma gramática simples e flexível (por exemplo, como filtragem, reordenamento, seleção, entre outras). Ele foi construído com o intuito de obter uma forma mais rápida e expressiva de tratar os dados. O `tibble` é a versão de data frame mais conveniente para se usar com pacote `dplyr`.

> b: eu penso mais como manipular dados, do que tratar
...
Todas as funções seguem a mesma sintaxe, de modo que o `tibble` é sempre o primeiro argumento dessas funções, seguido de um pipe e pelo nome da função. Isso permite o encadeamento de várias operações consecutivas.

1.  tibble
2.  operador pipe
3.  nome da função verbal com os argumentos entre parênteses
4.  outras funções verbais...

> b: acho que esses topicos ficaram meio perdidos

As funções verbais não precisam modificar necessariamente o tibble original, sendo que as operações de manipulações podem e devem ser atribuídas a um novo objeto.
...
> b: seria legal mostrar como era e como ficou.
...
> b: acho legal comentar o codigo pra descrever que filltros estao sendo feitos
...
```{r fig-r-join, fig.align='center', fig.cap="Diferentes tipos de joins, representados com um diagrama de Venn. Adaptado de: @wickham2017.", out.width="80%", echo=FALSE}
...
Para se aprofundar no tema, recomendamos a leitura do capítulo [13 Relational data](https://r4ds.had.co.nz/relational-data.html) de Wickham & Grolemund (2017).
...
Para se aprofundar no tema, recomendamos a leitura do capítulo [14 Strings](https://r4ds.had.co.nz/strings.html) de Wickham & Grolemund (2017).
...
```{r}
# Agregalção de níveis raros em um nível
forcats::fct_lump(penguins$species) %>% head()
```

> b: teve um tipo em `agregalção` 
...
Para se aprofundar no tema, recomendamos a leitura do capítulo [15 Factors](https://r4ds.had.co.nz/factors.html) de Wickham & Grolemund (2017).
...
> B:  ele instala sim, mas não carrega por padrão. veja aqui: https://github.com/tidyverse/tidyverse/blob/195d8a45d7ac425bb29043ca418538753b79a1b8/DESCRIPTION#L37
...
tempo_estudando_r_dur <- lubridate::as.duration(tempo_estudando_r)
 ...
 # >B: codigos nesse chunk, em vez de usar esses numeros, pq nao usa o tempo_estudando_r ?
...
Para se aprofundar no tema, recomendamos a leitura do capítulo [16 Dates and times](https://r4ds.had.co.nz/dates-and-times.html) de Wickham & Grolemund (2017).
...
> B: e os resultados?? seria legal ver. uma outra que é daora demais é purrr::map_dfr(), muito util para importar varios arquivos de mesma estrutura 
e unir em um unico df

Para se aprofundar no tema, recomendamos a leitura do capítulo [21 Iteration](https://r4ds.had.co.nz/iteration.html) de Wickham & Grolemund (2017).
...
> B: Aqui acho que dá pra colocar o nome dos autores mesmo, em vez de Curso-R..

...
Primeiramente, criaremos um diretório com a função `create.dir()`, usando a função `here::here()` para indicar o repositório (ver seção xx).
...
legend(x = -.05, y = .15, pch = 15, cex = .7, pt.cex = 2.5, 
...
Saber a melhor forma de exportar dados geoespaciais de objetos recém-criados no R é fundamental, principalmente porque essa ação irá depender do tipo de dado (vetor ou raster), classe do objeto (por exemplo, `MULTIPOINT` ou `RasterLayer`) e tipo e quantidade de informações armazenadas (por exemplo, tamanho do objeto, intervalo de valores, etc.).
...
Ainda sobre o formato `GeoPackage`, há algo muito interessante que podemos fazer: podemos acrescentar outros arquivos vetoriais ao mesmo arquivo já criado. Como exemplo, iremos exportar o limite do Brasil para o mesmo arquivo.
...
Outros argumentos de suporte são: `overwrite` para sobreescrever um arquivo que já exista, `progress` para mostrar uma barra de progresso da exportação como "text" ou "window", e `options` que permite opções do GDAL. Para esse último , quando exportar especificamente na extensão `GeoTIFF`, podemos utilizar `options = c("COMPRESS=NONE", "TFW=YES")` para que haja compressão do arquivo, diminuindo consideravelmente seu tamanho (cerca de um terço), aliado à criação de um arquivo auxiliar `.tfw`, para ser carregado em softwares específicos de SIG, como o ArcGIS.
...
Nesta seção veremos as principais funções para realizar operações com dados geoespaciais. Essas operações são separadas conforme Lovelace, Nowosad & Muenchow (2019) em: **Operações de atributos**, **Operações espaciais**, e **Operações geométricas**.
...
```{r fig-vetor-fig-vetor-distancia-espacial, fig.align='center', fig.cap="Distância espacial das nascentes até o fragmento de floresta mais próxima no município de Rio Claro/SP."}
# Plot
...
A operação zonal é realizada através da função `raster::zonal()`, que recebe de entrada no  **x** o raster contínuo, em **z** o raster categórico, e em **fun** a função que irá resumir as células. Em nosso exemplo, vamos calcular diversas medidas resumo da elevação com a função `summary()` para cada classe de elevação que criamos anteriormente.
...
```{r fig-vetor-pontos-aleatórios, fig.align='center', fig.cap="Sorteio de 30 pontos aleatoriotório para Rio Claro/SP."}
# Plot
plot(rc_2019_sirgas2000_utm23s$geom, col = "gray", main = NA, axes = TRUE, graticule = TRUE)
...
```{r fig-vetor-convexo, fig.align='center', fig.cap="Polígono convexo para os 30 pontos criados aleatoriotoriamente para Rio Claro/SP."}
# Plot
plot(rc_2019_sirgas2000_utm23s$geom, col = "gray", main = NA, axes = TRUE, graticule = TRUE)
...
```{r fig-vetor-voronoi, fig.align='center', fig.cap="Polígonos de Voronoi para os 30 pontos criados aleatoriotoriamente para Rio Claro/SP."}
# Plot
plot(rc_2019_sirgas2000_utm23s$geom, col = "gray", main = NA, axes = TRUE, graticule = TRUE)
...
```{r fig-raster-vetorizacao, fig.align='center', fig.cap="Vetorização do raster de elevação criando um gride de pontos para Rio Claro/SP."}
# Plot
...
5.  indicador de orientação (Norte) (indicando o norte geográfico, podendo ser representado por uma flecha, bússula ou compasso)
...
Podemos optar para ser mais restritos e selecionar apenas os hexágonos dentro do limite do Bioma da Mata Atlântica utilizando o operador `st_within()`.
...
```{r fig-vetor-aa-ma-sel-hex, fig.align='center', fig.cap="Mapa dos hexágonos totalmente dentro do limite da Mata Atlântica.", out.width="100%", warning=FALSE}
# Mapa
tm_shape(mata_atlantica, bbox = mata_atlantica_hex) +
  tm_polygons() +
...
Aqui, usaremos os dados do [GlobCover](http://due.esrin.esa.int/page_globcover.php) v.2.3 de 2009 [@arino2012] como raster de cobertura da terra. O arquivo é grande (~400 Mb) e pode demorar muito, dependendo da velocidade da internet.
...
download.file(url = "http://due.esrin.esa.int/files/Globcover2009_V2.3_Global_.zip",
              destfile = here::here("dados", "raster", "Globcover2009_V2.3_Global_.zip"), mode = "wb", 
extra = "c")
...
Depois de fazer o download, vamos importar e ajustar esse raster para o limite da Mata Atlântica, com a mesma resolução dos dados bioclimáticos da seção anterior. 

```{r}
# Importar raster do GlobCOver
ra_globcover <- raster::raster(here::here("dados", "raster", "GLOBCOVER_L4_200901_200912_V2.3.color.tif"))
...
Vamos agora transformar a tabela de locais em vetor, selecionar para dentro do Bioma da Mata Atlântica, selecionar aleatoriamente 50 amostragens das comunidades de anfíbios e criar buffers de ~10 km.
...
Podemos conferir no mapa da Figura \@ref(fig-vetor-aa-ma-buffer).
...
Nosso próximo passo é rasterizar esses buffers. Para isso usaremos a função `raster::rasterize()`, sendo que os valores dos rasters dos buffers seguiram as identificações das amostragens.

```{r}
# Rasterização
aa_especies_locais_ve_ma_buffer10km_rast <- raster::rasterize(x = aa_especies_locais_ve_ma_buffer10km, y = ra_globcover_ma, field = "id")
```

Agora podemos utilizar a função `raster::zonal()` para fazer a contabilização de pixels de cada classe para cada buffer.

```{r}
# Estatística zonal 
uso_buffer_freq <- raster::zonal(x = ra_globcover_ma, 
                                 y = aa_especies_locais_ve_ma_buffer10km_rast,
                                 fun = count,
                                 na.rm = TRUE)
...
### Gerando rasters de dados socio-econômicos

Muitas vezes precisamos criar rasters derivadas de informações tabulares, que geralmente são informações não-espacializadas. Aqui, iremos explorar como usar dados socio-econômicas de municípios para criar rasters que poderiam ser usados em Modelos de Distribuição de Espécies. Vamos baixar dados sócio-econômicos do [**Sistema IBGE de Recuperação Automática - SIDRA**](https://sidra.ibge.gov.br), fazer a associação para os polígonos dos municípios e depois rasterizar com base em uma variável do WorldClim.
...
Vamos fazer algumas manipulações de dados: filtros algumas linhas, transformar os dados, selecionar algumas colunas e renomar outras para que a associação com os municípios seja mais simples. No final, combinamos as colunas para simplificar os dados em um único `tibble`.
...
```{r eval=FALSE}
# Stack
mun_raster <- raster::stack(mun_agua_raster, 
...
<!--chapter:end:15_visualizacao_dados_geoespaciais_rev.Rmd-->

# Dados geoespaciais no R {#dados-geoespaciais}
...
Nesta seção, vamos fazer uma breve introdução aos principais conceitos sobre o funcionamento de dados geoespaciais no R: formatos de dados vetoriais e pacotes; formatos de dados raster e pacotes; Sistemas de Referências de Coordenadas e unidades; fontes de dados geoespaciais e pacotes; importar e exportar dados geoespaciais; descrição de objetos espaciais; reprojeção de dados geoespaciais; e principais operações com dados geoespaciais. Num segundo momento, criaremos mapas com seus principais elementos. Por fim, apresentaremos exemplos de aplicações de análises espaciais para dados ecológicos, focadas em resumir informações sobre a biodiversidade, preparar de dados para compor variáveis preditoras, e como fazer predições espaciais contínuas de distribuição de uma espécie e riqueza de espécies.

Esse capítulo segue parte da estrutura organizada por @lovelace2019, principalmente os capítulos 2 a 8, sendo adaptado para atender aos principais requisitos que julgamos necessários a estudos ecológicos. Entretanto, não foi possível cobrir todos os assuntos sobre geoprocessamento, sendo um assunto muito extenso que requer a leitura de livros especializados na área como @wegmann2016, @wegmann2020 e @fletcher2018. Outros livros sobre a análise geoespacial no R podem ser consultados no capítulo [11 - Geospatial](https://www.bigbookofr.com/geospatial.html) do [Big Book of R](https://www.bigbookofr.com/index.html).

## Vetor

Dados vetoriais representam informações geográficas acuradas através de pontos, linhas e polígonos (Figura \@ref(fig:fig-vetor-tipos)). Cada uma dessas geometrias são indicadas para representar feições e/ou eventos específicos, par de coordenadas (longitude - X e latitude - Y), estradas ou rios, e limites geográficos naturais, políticos, administrativos ou regulares.

```{r fig-vetor-tipos, echo=FALSE, fig.height=4, fig.align='center', fig.cap="Ilustração das geometrias de ponto, linha e polígono genéricos. Adaptado de: @lovelace2019."}
...
Para os dados vetoriais é necessário destacar um elemento fundamental: a tabela de atributos. A tabela de atributos é uma tabela que inclui dados geográficos e dados alfanuméricos. Os dados geoespaciais são representados por feição geolocalizada espacialmente (ponto, linha ou polígono), e os dados alfanuméricos. Dessa forma, a tabela de atributos reúne informações sobre cada feição e pode ser utilizada para realizar de filtros ou agregações dos dados de cada feição.

### sf: principal pacote no R para dados vetoriais

Atualmente o principal pacote para trabalhar com dados vetoriais no R é o [`sf`](https://r-spatial.github.io/sf/), que implementou o *Simple Feature* no R [@sf2018]. Entretanto, outro pacote pode ser tão versátil quanto o `sf`, no caso o [`terra`](https://rspatial.org/terra/index.html), ainda em desenvolvimento.
...
> IMPORTANTE: faremos mapas mais elaborados na seção \@ref(visualização-de-dados-geoespaciais) desse capítulo.
...
```{r fig-raster, echo=FALSE, fig.align='center', fig.cap="Raster: (A) IDs das células, (B) valores das células, (C) células coloridas. Adaptado de: @lovelace2019."}
library(gridExtra)
...
Os dados geoespaciais (vetor e raster) possuem ainda um outro componente fundamental que é o Sistema de Referência de Coordenadas, ou do inglês *Coordinate Reference System (CRS)*. Esse componente define como os elementos espaciais (vetor e raster) representam uma feição na superfície da Terra. Ele é composto por dois principais conceitos: primeiro, o tipo de unidades estão sendo utilizadas para a representação geográfica, podendo assumir dois tipos - ângulos ou metros, que definem o Sistema de Coordenadas Geográficas e o Sistema de Coordenadas Projetadas, respectivamente.

O segundo componente é o datum, que é a relação do sistema de coordenadas (geográfica ou projetada) com a superfície da Terra. Esse último componente faz parte de uma área da Cartografia denominada Geodésia que estuda a forma e dimensões da Terra, campo gravitacional e a localização de pontos fixos e sistemas de coordenadas. O livro de @lapaine2017 é um excelente material para se aprofundar nesse assunto.

### Sistema de Coordenadas Geográficas

O Sistema de Coordenadas Geográficas utiliza ângulos (graus) para representar feições na superfície da Terra através de dois valores: longitude e latitude. A longitude localiza-se na direção Leste-Oeste e a latitude na direção Norte-Sul. Nesse sistema, a superfície da Terra é representada geralmente por uma superfície elipsoidal, pois a Terra é ligeiramente achatada nos polos.
...
Primeiramente, criaremos um diretório com a função `create.dir()`, usando a função `here::here()` para indicar o repositório (ver a seção \@ref(here) do Capítulo \@ref(tidyverse)).
...
```{r fig-vetor-hidrografia, fig.align='center', fig.cap="Mapa da hidrografia de Rio Claro/SP."}
# Plot
...
```{r tab-vetor-dados-geobr, echo=FALSE, eval=FALSE}
kable(head(geobr::list_geobr()),
...
```{r fig-vetor-rio-claro, fig.align='center', fig.cap="Limite do município de Rio Claro/SP."}
# Plot
...
```{r fig-vetor-brasil, fig.align='center', fig.cap="Limite do Brasil."}
# Plot
...
Para tanto, vamos usar os dados de comunidades de anfíbios da Mata Atlântica (Atlantic Amphibians, Vancine et al. 2018). Fazeremos o download diretamente do site da fonte dos dados. Antes vamos criar um diretório.
...
```{r fig-vetor-pontos-atlantic-amphibians, fig.align='center', fig.cap="Coordenadas das comunidades do Atlantic Amphinians (Vancine et al. 2018)."}
# Plot
...
```{r fig-raster-dem, fig.align='center', fig.cap="Camada raster do DEM em relação ao limite do município de Rio Claro/SP."}
# Plot
...
Além dos dados de elevação, dados de temperatura e precipitação podem ser obtidos do [WorldClim](https://www.worldclim.org/). Para saber mais sobre esses dados, recomendamos a leitura do artigo Fick & Hijmans (2017).
...
```{r fig-raster-wc, fig.align='center', fig.cap="Camadas rasters do WorldClim (BIO01 e BIO12) para o mundo."}
# Plot
...
knitr::kable(tibble(
  ...
  spatialreference.org = c("[EPSG:4326](https://spatialreference.org/ref/epsg/4326/)",
                           "[EPSG:4674](https://spatialreference.org/ref/epsg/4674/)",                        "[SR-ORG:7099](https://spatialreference.org/ref/sr-org/7099/)",                    "[SR-ORG:7291](https://spatialreference.org/ref/sr-org/7291/)",                    "[ESRI:54012](https://spatialreference.org/ref/esri/54012/)",                      NA,
...
```{r fig-vetor-crs-trans, echo=FALSE, fig.align='center', fig.cap="Limites do município de Rio Claro/SP com CRS SIRGAS2000/geográfico e com CRS SIRGAS2000/UTM23S."}
...
```{r fig-vetor-mundo-wgs84, fig.align='center', fig.cap="Camada BIO01 para o mundo com CRS geográfico e datum WGS84."}
# Plot
...
```{r fig-vetor-mundo-moll, fig.align='center', fig.cap="Camada BIO01 para o mundo com CRS Projeção de Mollweide."}
# Plot
...
```{r fig-vetor-mundo-laea, fig.align='center', fig.cap="amada BIO01 para o mundo com CRS Projeção Azimutal de Lambert centrado no Brasil."}
# Plot
...
Aqui, vamos reprojetar os dados de elevação para Rio Claro/SP. Para que esse processo seja mais rápido, iremos antes ajustar a extensão do raster para o limite do município usando a função `raster::crop()` (Figura \@ref(fig:fig-raster-crop)). Essa função é melhor explicada na seção \@ref(cortes-e-máscaras).
...
```{r fig-raster-reproj, fig.align='center', fig.cap="Reprojeção do raster de elevação para SIRGAS2000/UTM23S especificado por um objeto e informando o tamanho da célula."}
# Plot
...
```{r fig-raster-reproj-celula-mundo, fig.align='center', fig.cap="Reprojeção do raster de média de temperatura anual (BIO01) para Projeção de Mollweide informando o tamanho da célula."}
# Plot
...
Quase todas as operações serão as mesmas realizadas pelo pacote `dplyr` em uma tabela de dados (ver a seção \@ref(dplyr) do Capítulo \@ref(tidyverse)), sendo algumas operações específicas para alterar apenas campos da tabela de atributos e outras que refletem operações nas feições, ou seja, alterarão através da tabela de atributos as características das feições. Essas funções e suas operações são descritas com detalhes na Tabela (\@ref(tab:tab-vetor-operacoes-atributos)).
...
```{r fig-vetor-opat-agregar, fig.align='center', fig.cap="Agregação e contagem das nascentes para o município de Rio Claro/SP."}
# Plot
plot(rc_2019_sirgas2000_utm23s$geom, col = "gray", main = NA, axes = TRUE, graticule = TRUE)
...
```{r fig-raster-stack-subset, fig.align='center', fig.cap="Camada BIO01 selecionada pelas operações de subconjunto."}
# Plot
...
```{r}
# Média de todas as células de altitude
raster::cellStats(x = ra_rc, stat = mean)

# Média de todas as células de cada camada bioclimática
...
Para exemplificar, vamos atribuir os valores dos polígonos de cobertura da terra aos pontos de nascentes para Rio Claro/SP, fazendo um agrupamento pela tabela de atributos para permitir criar o mapa da Figura \@ref(fig:fig-vetor-junção-espacial).
...
```{r fig-vetor-junção-espacial, fig.align='center', fig.cap="Junção espacial da cobertura da terra para as nascentes no município de Rio Claro/SP."}
# Plot
plot(rc_2019_sirgas2000_utm23s$geom, col = "gray", main = NA, axes = TRUE, graticule = TRUE)
...
```{r fig-vetor-agregacao-espacial, fig.align='center', fig.cap="Agregação espacial contabilizando o número de nascentes para cada classe de cobertura da terra no município de Rio Claro/SP."}
# Plot
...
```{r fig-vetor-distancia-espacial, fig.align='center', fig.cap="Distância espacial das nascentes até o fragmento de floresta mais próxima no município de Rio Claro/SP."}
# Plot
plot(rc_nas_dist_flo[7], pch = 20, axes = TRUE, graticule = TRUE, main = NA)
```
...
ra_rc_abaixo_500 <- raster::calc(x = ra_rc, 
...
A simplificação possui o intuito de generalizar linhas ou polígonos, diminuindo assim suas complexidades em relação ao número de vértices. É utilizada para representação em mapas menores ou mapas interativos (seção \@ref(mapas-interativos)), ou ainda quando um objeto vetorial é muito grande. A função utilizada é a `sf::st_simplify()`, que usa o argumento `x` para uma geometria de entrada e `dTolerance` para controlar o nível de generalização nas unidades do mapa. Em nosso exemplo, simplificaremos a hidrografia de Rio Claro/SP (Figura \@ref(fig:fig-vetor-simplificacao)).
...
```{r fig-vetor-centroide, fig.align='center', fig.cap="Centroide do limite do município de Rio Claro/SP."}
# Plot
plot(rc_2019_sirgas2000_utm23s$geom, col = "gray", main = NA, axes = TRUE, graticule = TRUE)
...
```{r fig-vetor-pontos-aleatorios, fig.align='center', fig.cap="Sorteio de 30 pontos aleatório para Rio Claro/SP."}
# Plot
plot(rc_2019_sirgas2000_utm23s$geom, col = "gray", main = NA, axes = TRUE, graticule = TRUE)
...
```{r fig-vetor-buffer, fig.align='center', fig.cap="Buffers de 1000 metros para os 30 pontos aleatórios no município de Rio Claro/SP."}
# Plot
plot(rc_2019_sirgas2000_utm23s$geom, col = "gray", main = NA, axes = TRUE, graticule = TRUE)
...
```{r fig-vetor-convexo, fig.align='center', fig.cap="Polígono convexo para os 30 pontos criados aleatoriamente para Rio Claro/SP."}
# Plot
plot(rc_2019_sirgas2000_utm23s$geom, col = "gray", main = NA, axes = TRUE, graticule = TRUE)
...
```{r fig-vetor-voronoi, fig.align='center', fig.cap="Polígonos de Voronoi para os 30 pontos criados aleatoriamente para Rio Claro/SP."}
# Plot
plot(rc_2019_sirgas2000_utm23s$geom, col = "gray", main = NA, axes = TRUE, graticule = TRUE)
...
```{r fig-vetor-quad, fig.align='center', fig.cap="Quadrículas de 2000 metros de arestas e centroides para Rio Claro/SP."}
# Plot
plot(rc_2019_sirgas2000_utm23s$geom, col = "gray", main = NA, axes = TRUE, graticule = TRUE)
...
```{r fig-vetor-hex, fig.align='center', fig.cap="Hexágonos equivalentes a quadrículas de 2000 metros de arestas e centroides para Rio Claro/SP."}
# Plot
plot(rc_2019_sirgas2000_utm23s$geom, col = "gray", main = NA, axes = TRUE, graticule = TRUE)
...
```{r fig-vetor-interseccao, fig.align='center', fig.cap="Recorte da hidrografia para dentro dos buffers dos 30 aleatórios para Rio Claro/SP."}
# Plot
plot(rc_2019_sirgas2000_utm23s$geom, col = "gray", main = NA, axes = TRUE, graticule = TRUE)
plot(rc_2019_sirgas2000_utm23s_pontos_aleatorios_buffer_uniao, col = adjustcolor("blue", .1), add = TRUE)
...
```{r fig-vetor-tipo, fig.align='center', fig.cap="Transformação do vetor de florestas em `POLYGON` e cálculo da área para cada feição para Rio Claro/SP."}
# Plot
plot(rc_2019_sirgas2000_utm23s$geom, col = "gray", main = NA, axes = TRUE, graticule = TRUE)
...
```{r fig-raster-desagregacao, fig.align='center', fig.cap="Desagregação (diminuição do pixel para 45 metros) utilizando o método bilinear para o raster de elevação para Rio Claro/SP."}
# Plot
...
```{r fig-raster-reamostragem, fig.align='center', fig.cap="Reamostragem (alinhamento dos rasters) utilizando o método bilinear para alinhar o raster bioclimático (BIO01) ao de elevação para Rio Claro/SP."}
# Plot
...
A função `raster::crop()` ajusta o raster à extensão do vetor. Como exemplo, vamos retomar o raster de elevação original baixado e importado na seção \@ref(raster-layer) (Figura \@ref(fig:fig-raster-dem)). Primeiramente, vamos usar a função `raster::crop()` para ajustar esse raster à extensão do limite do município de Rio Claro/SP (Figura \@ref(fig:fig-raster-corte)).
...
```{r fig-raster-mascara, fig.align='center', fig.cap="Ajuste do raster de elevação para o limite de Rio Claro/SP."}
# Plot
...
```{r fig-raster-corte-mascara-inverso, fig.align='center', fig.cap="Ajuste da extensão e do limite externo do raster de elevação para Rio Claro/SP."}
# Plot
...
```{r fig-raster-rasterizacao-pontos, fig.align='center', fig.cap="Rasterização das nascentes, com a operação de contabilização de pontos para Rio Claro/SP."}
# Plot
...
```{r fig-raster-rasterizacao-linhas, fig.align='center', fig.cap="Rasterização da hidrografia, com a operação de contabilização de linhas para Rio Claro/SP."}
# Plot
...
```{r fig-raster-rasterizacao-poligonos, fig.align='center', fig.cap="Rasterização da cobertura da terra para Rio Claro/SP."}
# Plot
...
```{r fig-raster-vetorizacao-pontos, fig.align='center', fig.cap="Vetorização do raster de elevação criando um gride de pontos para Rio Claro/SP."}
# Plot
raster::plot(ra_rc_sirgas2000_utm23s_agre_media, 
             col = viridis::viridis(10, alpha = .8))
plot(ra_rc_sirgas2000_utm23s_agre_media_pontos, 
     pch = 20, cex = .7, main = FALSE, add = TRUE)
plot(rc_2019_sirgas2000_utm23s$geom, col = NA, 
...
raster::plot(ra_rc_sirgas2000_utm23s_agre_media,
             col = viridis::viridis(10, alpha = .8))
contour(ra_rc_sirgas2000_utm23s_agre_media, labcex = 1, 
        main = FALSE, add = TRUE)
plot(rc_2019_sirgas2000_utm23s$geom, col = NA, border = "red", 
     lwd = 2, add = TRUE)
```
...
```{r fig-raster-vetorizacao-poligonos, fig.align='center', fig.cap="Vetorização do raster de cobertura da terra para Rio Claro/SP, não dissolvendo e dissolvendos os polígonos gerados."}
# Plot
old_par <- par(mfrow = c(1, 2))
raster::plot(rc_cob_rasterizacao, col = viridis::viridis(10))
...
4.  Legenda (apresentando as informações detalhadas das classes ou escala de valores, geralmente identificando as cores e/ou texturas), barra de escala (representando quantas unidades do mapa representam do mundo real)
5.  Indicador de orientação (Norte) (indicando o norte geográfico, podendo ser representado por uma flecha, bússola ou compasso)
6.  Gride de coordenadas (coordenadas presentes nas laterais)
7.  Descrição do CRS (indicando qual o CRS)
8.  Informações de origem (informações sobre a fonte dos dados representados no mapa)
9.  Além de outros elementos auxiliares (como elementos textuais e figuras extras)
...
```{r}
# Download de polígonos dos Biomas Brasileiros
...
```{r fig-raster-layer-mapa, fig.align='center', fig.cap="Mapa feito com a função `plot()` de um objeto raster com uma camada.", out.width="100%"}
# Plot
plot(ra_rc)
plot(rc_2019$geom, col = NA, border = "red", lwd = 2, add = TRUE)
```
...
```{r fig-raster-stack-mapa, fig.align='center', fig.cap="Mapa feito com a função `plot()` de um objeto raster com várias camadas.", out.width="100%"}
# Plot
...
Como discutimos no Capítulo \@ref(graficos) sobre gráficos, o pacote `ggplot2` utiliza a gramática de gráficos para composição de figuras no R [@wilkinson2005, @ggplot22016]. Para cada classe de objeto geográfico há funções específicas para os dados: para objetos sf `geom_sf()` e para objetos raster `geom_raster()`.
...
A estrutura de composição das funções do pacote `ggplot2` vai funcionar parecido com a estruturação de gráficos já vista no capítulo \@ref(graficos), de modo que a cada função iremos utilizando o sinal de `+` para acrescentar outra camada. Indicaremos os dados com a função `ggplot()` e a coluna da tabela de atributos que queremos representar com a função `aes()`. Em seguida, utilizamos a função `geom_sf()` para indicar que trata-se de um objeto `sf`.
...
```{r fig-vetor-mapa-ggplot2, fig.align='center', fig.cap="Mapa de Biomas do Brasil com o pacote `ggplot2`.", out.width="100%"}
# Plot
...
```{r fig-vetor-mapa-tmap, fig.align='center', fig.cap="Mapa de Biomas do Brasil com o pacote `tmap`.", out.width="100%"}
# Mapa
...
```{r fig-raster-mapa-tmap, fig.align='center', fig.cap="Mapa raster de elevação com o pacote `tmap`.", out.width="100%"}
# Mapa
...
# Número de Estados ao longo do tempo
br_anos$year %>% table
...
```{r fig-vetor-brasil-tmap-animated, fig.align='center', fig.cap="Mapa vetorial animado mostrando os estados brasileiros ao longo do tempo com o pacote `tmap`.", out.width="100%", echo=FALSE, warning=FALSE}
knitr::include_graphics("img/cap14_fig78.gif")
```
...
knitr::include_url("img/cap14_fig79.html")
```
...
knitr::include_url("img/cap14_fig80.html")
```
...
```{r}
# Importar espécies
aa_especies <- readr::read_csv(here::here("dados", "tabelas", "ATLANTIC_AMPHIBIANS_species.csv"), col_types = cols()) %>%
...
aa_especies_locais_ve <- aa_especies %>% 
  dplyr::left_join(aa_locais, by = "id") %>% 
...
mata_atlantica_hex_total <- mata_atlantica_hex[mata_atlantica, , op = st_within]
```

```{r fig-vetor-aa-ma-sel-hex-total, fig.align='center', fig.cap="Mapa dos hexágonos totalmente dentro do limite da Mata Atlântica.", out.width="100%", warning=FALSE}
# Mapa
tm_shape(mata_atlantica, bbox = mata_atlantica_hex) +
  tm_polygons() +
  tm_shape(mata_atlantica_hex_total) +
  tm_borders()
```
...
Já realizamos o download das variáveis bioclimáticas na seção \@ref(raster-stack). Vamos importar novamente esses dados, primeiramente listando as camadas e depois importando com a função `raster:stack()`.
...
download.file(url = "http://due.esrin.esa.int/files/Globcover2009_V2.3_Global_.zip",
              destfile = here::here("dados", "raster", "Globcover2009_V2.3_Global_.zip"), 
              mode = "wb", extra = "c")

# Unzip
unzip(zipfile = here::here("dados", "raster", "Globcover2009_V2.3_Global_.zip"),
      exdir = here::here("dados", "raster"))
```
...
```{r fig-raster-globcover, fig.align='center', fig.cap="Camada raster do GlobCover 2.3 para o Bioma da Mata Atlântica."}
# Plot
plot(ra_globcover_ma)
```
...
aa_especies_locais_ve_ma <- aa_locais %>% 
  sf::st_as_sf(coords = c("longitude", "latitude"), crs = 4326) %>%
  dplyr::filter(sf::st_intersects(x = ., y = mata_atlantica, sparse = FALSE)) %>% 
...
```{r fig-vetor-aa-ma-buffer, fig.align='center', fig.cap="Distribuição de 50 localidades aleatórios e buffers de ~10 km (fora de escala).", out.width="100%", warning=FALSE}
# Mapa
tm_shape(mata_atlantica) +
  tm_polygons() +
  tm_shape(aa_especies_locais_ve_ma_buffer10km) +
...
```{r}
# Estatística zonal 
aa_locais_ve_ma_buffer10km_ext <- raster::extract(
  x = ra_globcover_ma,
...
head(aa_locais_ve_ma_buffer10km_ext)
```
...
```{r}
# Junção
aa_locais_ve_ma_buffer10km_ext_join  <- dplyr::left_join(
  x = aa_especies_locais_ve_ma_buffer10km,
  y = aa_locais_ve_ma_buffer10km_ext,
  by = "id") 
aa_locais_ve_ma_buffer10km_ext_join
```

### Gerando rasters de dados tabulares

Muitas vezes precisamos criar rasters derivadas de informações tabulares, que geralmente são informações não-espacializadas. Aqui, exploraremos como usar dados socioeconômicas de municípios para criar rasters que poderiam ser usados em Modelos de Distribuição de Espécies. Vamos baixar dados sócioeconômicos do [**Sistema IBGE de Recuperação Automática - SIDRA**](https://sidra.ibge.gov.br), fazer a associação para os polígonos dos municípios e depois rasterizar com base em uma variável do WorldClim.
...
O banco de dados do SIDRA é absurdamente enorme. Aqui vamos explorar apenas uma tabela, mas existem muitas possibilidades para explorar.
...
Vamos selecionar a tabela "Tabela 3218: Domicílios particulares permanentes, por forma de abastecimento de água, segundo a existência de banheiro ou sanitário e esgotamento sanitário, o destino do lixo e a existência de energia elétrica".
...
Agora começam as complicações, pois o API só permite o download de 50000 linhas de dados. Então, teremos que fazer downloads específicos para as diferentes categorias.
...
Vamos fazer algumas manipulações de dados: filtros algumas linhas, transformar os dados, selecionar algumas colunas e renomear outras para que a associação com os municípios seja mais simples. No final, combinamos as colunas para simplificar os dados em um único `tibble`.
...
Agora vamos baixar os limites políticos dos municípios. Vamos usar os dados oficiais do IBGE, baixando os dados para o mesmo ano (2010), usando a função `geobr::read_municipality()` do pacote [`geobr`](https://cran.r-project.org/web/packages/geobr/index.html).

```{r message=FALSE, warning=FALSE}
# Vetor
br_mun <- geobr::read_municipality(code_muni = "all", year = 2010, showProgress = FALSE)
```

Agora que temos os dois dados (tabular e espacial), vamos fazer a associação entre eles usando a função `dplyr::left_join()` do pacote [`dplyr`](https://dplyr.tidyverse.org/), usando a coluna "code_muni" como a coluna chave.

```{r}
# Junção
br_mun_da <- dplyr::left_join(x = br_mun, y = da_data, by = "code_muni")

# Dados
dplyr::glimpse(br_mun_da)
```

Agora que temos os limites dos municípios com os dados associados, vamos usar um raster de modelo para rasterizar os polígonos. Para isso, vamos utilizar os dados que já baixamos e importamos na seção \@ref(raster-stack).

Também vamos adequar o raster do WorldClim para a extensão e limite do Brasil com as funções `raster::crop()` e `raster::mask()`, respectivamente (\@ref(fig:fig-raster-bio-br)).

```{r message=FALSE, warning=FALSE}
# Extensão e limite
var_br <- var$bio01 %>% 
  raster::crop(br_mun_da) %>% 
  raster::mask(br_mun_da)
```

```{r fig-raster-bio-br, fig.align='center', fig.cap="Adequação do limite da camada BIO01 ao limite do Brasil.", out.width="100%", warning=FALSE}
# Mapa
tm_shape(var_br) +
...
Agora sim, podemos fazer a rasterização dos polígonos dos municípios usando o raster que criamos como "molde", usando a função `fasterize::fasterize()`, também do pacote [`fasterize`](https://cran.r-project.org/web/packages/fasterize/index.html).

```{r}
# Rasterizar
var_br_agua_raster <- fasterize::fasterize(sf = br_mun_da, raster = var_br, field = "agua_rede")

var_br_sem_banheiro_raster <- fasterize::fasterize(sf = br_mun_da, raster = var_br, field = "sem_banheiro")

var_br_lixo_raster <- fasterize::fasterize(sf = br_mun_da, raster = var_br, field = "lixo_coletado")

var_br_luz_raster <- fasterize::fasterize(sf = br_mun_da, raster = var_br, field = "luz")
```

Vamos fazer os mapas para ver os resultados (\@ref(fig:fig-raster-bio-br-vars)).

```{r fig-raster-bio-br-vars, fig.align='center', fig.cap="Rasterização dos polígonos dos municípios usando o raster da camada BIO01.", out.width="100%", warning=FALSE}
# Mapas
map_agua <- tm_shape(var_br_agua_raster) +
  tm_raster(pal = "Blues", title = "Água de rede geral (%)") +
  tm_layout(legend.position = c("left", "bottom"))

map_banheiro <- tm_shape(var_br_sem_banheiro_raster) +
  tm_raster(pal = "RdYlGn", title = "Sem banheiro (%)") +
  tm_layout(legend.position = c("left", "bottom"))

map_lixo <- tm_shape(var_br_lixo_raster) +
  tm_raster(pal = "Greys", title = "Lixo coletado (%)") +
  tm_layout(legend.position = c("left", "bottom"))

map_luz <- tm_shape(var_br_luz_raster) +
...
```{r eval=FALSE}
# Stack
var_br_raster <- raster::stack(var_br_agua_raster, 
                               var_br_sem_banheiro_raster, 
                               var_br_lixo_raster, 
                               var_br_luz_raster)

# Exportar
raster::writeRaster(x = var_br_raster, 
                    filename = "var",
...
Agora sim, podemos ajustar um modelo simples da presença e ausência dessa espécie, utilizando as variáveis não correlacionadas, através de um GLM para a família binomial (para mais detalhes volte para o capítulo \@ref()).
...
```{r fig-raster-bio-ajuste, fig.align='center', fig.cap="Mapa de dois rasters (BIO01 e BIO12) ajustados ao limite da Mata Atlântica.", out.width="100%"}
# Mapa
...
```{r}
# Predições
modelo_pa_pred <- raster::predict(model = modelo_pa, object = var_mata_atlantica, type = "response")
modelo_pa_pred
```
...
modelo_pa_pred_corte <- modelo_pa_pred >= 0.01
```

Por fim, vamos produzir dois mapas mostrando os valores das predições e o mapa binário. \@ref(fig:fig-raster-pred-modelo-bin)).
...
# Ocorrências de haddadus binotatus
...
mapa_pred_bin <- tm_shape(modelo_pa_pred_corte) +
  tm_raster(title = "Predição binária", pal = c("#469fb3", "#d53e4f"), 
...
```{r fig-raster-pred-modelo-riq, fig.align='center', fig.cap="Mapa da predição de riqueza utilizando o modelo Poisson e Binomial Negativa para a Mata Atlântica.", out.width="100%", warning=FALSE}
# Mapa da predição Poisson
mapa_pred_riq_pois <- tm_shape(modelo_riq_pois_contagem) +
...
mapa_pred_riq_nb <- tm_shape(modelo_riq_nb_contagem) +

...
Podemos ainda **renomear colunas** facilmente com a função `dplyr::rename()` e suas variantes `dplyr::rename_with` ou com os sufixos `*_at()`, `*_if()`, `*_all()`, que muito interessantes para mudanças em várias colunas de uma vez.
...
Outra operação bastante usual dentro da manipulação de dados tabulares é a **seleção de colunas**. Podemos fazer essa operação com a função `dplyr::select()`, que seleciona colunas pelo nome ou pela posição. Aqui há uma séria de possibilidades de seleção de colunas, que listamos na Tabela (\@ref(tab:tab-dplyr-select)).
...
Quando usamos a função `dplyr::select()`, mesmo que para uma linha, o retorno é sempre um `tibble`. Caso precisemos que essa **coluna se torne um vetor**, dentro so encadeamento dos pipes, usamos a função `dplyr::pull()` que **que extrai uma coluna única coluna como vetor**.
...
Uma das operações mais úteis dentre as operações para colunas é **adicionar ou atualizar os valores** de colunas. Para essa operação, usaremos a função `dplyr::mutate()`. Essa função irá adiconar colunas ou atualizar uma coluna fazendo operações em outras colunas. Existe uma séria de funções que podemos utilizar para fazer essas operações. Destacamos aqui a função `dplyr::across()`, que aplica uma função em múltiplas colunas. Além dessa função, podemos ainda usar os argumentos **.before** e **.after** para indicar onde a nova coluna deve ficar. Temos ainda o parâmetro **.keep** com diversas possibilidades de manter colunas depois de usar a função `dplyr::mutate()`.
...
Além de operações em colunas, podemos fazer **operações em linhas**. Vamos começar com a **reordenação das linhas** com base nos valores de colunas. Para essa operação, usamos a função `dplyr::arrange()`. `desc()`
**.by_group**

`dplyr::filter()`: seleciona linhas com base em valores


`dplyr::distinct()`: remove linhas com valores repetidos com base nos valores de colunas


`dplyr::slice()`: seleciona linhas pelos números


`dplyr::sample_n()`: amostragem aleatória de linhas


`dplyr::summarise()`: agrega ou resume os dados através de funções, podendo considerar valores das colunas


`dplyr::*_join()`: junta dados de duas tabelas através de uma coluna chave
